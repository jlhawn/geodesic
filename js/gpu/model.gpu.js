 ...
 * `step` only queues work (every read-back waits for it), `diagnostics`
 * reads the per-cell energy terms back and sums them on the CPU, and
 * `ocean` carries the same initialize/load/serialize contract. `sync`
 * also copies the GPU's layer Exner function and geopotential into a CPU
 * sigma core on the same mesh, whose level interpolation serves the page.
 */
export async function createGpuModel(gridOrMesh, {
  ...
  async function sync() {
    ...
    const { exnerLayer, geopotential } = await gpu.downloadDiagnosis();
    core.diagnostics.exnerLayer.set(exnerLayer);
    core.diagnostics.geopotential.set(geopotential);
    dirty = false;
  }
  model.sync = sync;
  ...
  /*
   * The diagnostics in two halves: the copies of everything they read are
   * queued now, with the rain and runoff accumulators restarting behind
   * them, and the returned promise refreshes the mirrors and sums once the
   * copies land, so a caller can queue more steps in between.
   */
  model.beginDiagnostics = function beginDiagnostics() {
    if (gpuOcean) gpuOcean.accumulateFreshwater(0);
    const stateRead = dirty ? gpu.download() : null, diagnosisRead = dirty ? gpu.downloadDiagnosis() : null;
    const physicsRead = gpu.downloadPhysics();
    const oceanRead = gpuOcean ? gpuOcean.download() : null;
    restartRunoff();
    ...
    const time = model.time, interval = time - lastPrecipTime;
    lastPrecipTime = time;
    dirty = false;
    return (async () => {
      if (stateRead) {
        const arrays = await stateRead;
        arrays.forEach((a, i) => state[i].set(a));
        const { exnerLayer, geopotential } = await diagnosisRead;
        core.diagnostics.exnerLayer.set(exnerLayer);
        core.diagnostics.geopotential.set(geopotential);
      }
      const ph = await physicsRead;
      refreshLand(ph);
      ...
      if (oceanRead) {
        const o = await oceanRead;
        lastOcean = o;
        Object.assign(result, gpuOcean.diagnosticsFrom(o));
      }
      return { diagnostics: result, time };
    })();
  };
  model.diagnostics = async function diagnostics() { return (await model.beginDiagnostics()).diagnostics; };
  ...
  model.land = landCpu ? {
    ...
    async serialize() { if (gpuOcean) gpuOcean.accumulateFreshwater(0); const read = gpu.downloadPhysics(); restartRunoff(); refreshLand(await read); return landCpu.serialize(); },
  } : null;
