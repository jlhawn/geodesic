...
// each and the <name>.parts.json manifest the page loads them from.
import { readFileSync, writeFileSync } from 'node:fs';
import { gzipSync } from 'node:zlib';

const [file, mib = '10'] = process.argv.slice(2);
if (!file) { console.error('usage: node scripts/splitState.mjs <state.json|state.bin[.gz]> [chunkMiB]'); process.exit(1); }
